<x-filament-panels::page>
    ...
    {{-- Report Table --}}
    <x-filament::section heading="QA Officer Performance">
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
                <thead class="text-xs uppercase bg-gray-50 dark:bg-gray-700">
                    <tr>
                        <th class="px-4 py-3">QA Officer</th>
                        <th class="px-4 py-3">Email</th>
                        <th class="px-4 py-3 text-center">Assignments</th>
                        ...
                        <th class="px-4 py-3 text-center">Clarification</th>
                ...
                <tbody>
                    @forelse ($this->getReportData() as $row)
                        <tr class="border-b dark:border-gray-700">
                            <td class="px-4 py-3 font-medium">{{ $row['qa_officer'] }}</td>
                            <td class="px-4 py-3 text-gray-500">{{ $row['email'] }}</td>
                            <td class="px-4 py-3 text-center">{{ $row['assignments'] }}</td>
                            ...
                            <td class="px-4 py-3 text-center text-yellow-600 font-semibold">{{ $row['clarification'] }}</td>
                            <td class="px-4 py-3 text-center text-gray-500">{{ $row['pending'] }}</td>
                ...
                @if ($this->getReportData()->isNotEmpty())
                    <tfoot class="font-semibold bg-gray-50 dark:bg-gray-700">
                        <tr>
                            ...
                            <td class="px-4 py-3 text-center">{{ $this->getReportData()->sum('assignments') }}</td>
                            <td class="px-4 py-3 text-center">{{ $this->getReportData()->sum('completed') }}</td>
                            ...
                            <td class="px-4 py-3 text-center text-yellow-600">{{ $this->getReportData()->sum('clarification') }}</td>
                            <td class="px-4 py-3 text-center">{{ $this->getReportData()->sum('pending') }}</td>
